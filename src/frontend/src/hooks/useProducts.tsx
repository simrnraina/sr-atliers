...
function mapCategory(cat: ProductCategory): Category {
  if (cat === "new-arrivals") return Category.NewArrivals;
  if (
    cat === "blazers" ||
    cat === "tops" ||
    cat === "bottoms" ||
  ...
  if (cat === "dresses") return Category.NewArrivals;
  return Category.NewArrivals;
}
...
function mapBackendProduct(p: BackendProduct): Product {
  return {
    ...
    images: [p.imageBlob.getDirectURL()],
...
// ─── Public hooks (fall back to sample data if actor unavailable) ─────────────
  ...
  return useMutation({
    mutationFn: async (data: AdminProductFormData) => {
      ...
      if (data.imageFile) {
        ...
        imageBlob = ExternalBlob.fromURL(data.imageUrl);
      } else {
        imageBlob = ExternalBlob.fromURL(
          "/assets/generated/blazer-beige.dim_800x1000.jpg",
        );
      }

      const backendCategory = mapCategory(data.category);
...
export function useUpdateProduct() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (data.imageFile) {
        const bytes = new Uint8Array(await data.imageFile.arrayBuffer());
        input.imageBlob = ExternalBlob.fromBytes(bytes);
      } else if (data.imageUrl) {
        input.imageBlob = ExternalBlob.fromURL(data.imageUrl);
      }
